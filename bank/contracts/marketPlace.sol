...
interface IMARKETPLACE {
    ...
    function sell(uint256 p_idNFT, uint256 p_price,string memory coin) external  returns (bool);
    ...
    function onSale(uint256 p_idNFT)
        external
        view
        returns (bool nftOnSale, uint256 priceOnSale, address oldOwnerSale,string memory coin);

   function buy(uint256 p_idNFT) external payable returns (bool, bool, uint256);
...
contract MarketPlaceForest is IMARKETPLACE {
    //////////////////////////////////////////////////////////////////////////////////////////////////
    // STATE
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    */
    ...
    /*
      KEY: Nft Id 
      VALUE: Transfer Nft By coin(BNB / LEAF)
    */
    mapping(uint256 => string) private s_salesCoin;
    ...
    */
    ...
    // Used to add erc20 token contract address
    address  private   UPDATE_TOKEN_ADDRESS;
  ...
  //  address private immutable TROOPS_ADDRESS;
    ...
    constructor(
      ...
      //  address p_troopsContract
        ...
        ERC721_ADDRESS = 0xEaE4c494e8f194d374b507230aA0E6531F7D9097;
        ERC20_ADDRESS = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        REWARDS_ADDRESS  = 0xE938A867bBf3Eb888FB634FB0D21e1aF99Ab9b2f;
        UPDATE_TOKEN_ADDRESS = msg.sender;
    ...
    //update erc20 token contract address
    function updateTokenAddress(address tokenAddress) public returns(bool){
        require(msg.sender == UPDATE_TOKEN_ADDRESS, "not allowd");
    ...
    // Put NFT on sale
    function sell(uint256 p_idNFT, uint256 p_price,string memory  coin)
        public  
        override
        returns (bool)
    {
...
/*    function cancelSale(uint256 p_idNFT) public override returns (bool) {
    ...
    */
    ...
    {
        return (s_sales[p_idNFT], s_salesPrice[p_idNFT],s_oldOwner[p_idNFT],s_salesCoin[p_idNFT]);
    ...
    // Buy one NFT
    function buy(uint256 p_idNFT) public payable override returns (bool,bool,uint256)  {
        require(_canBuy(p_idNFT), "Not allowed");

        bytes memory _b = bytes("BNB");
        bytes memory _a = bytes(s_salesCoin[p_idNFT]);
       if(keccak256(_a) == keccak256(_b)){
            require(msg.value >= s_salesPrice[p_idNFT], "no enough balance");
            // 2% bnb fee to rewards contract
            uint256 amountFee = (s_salesPrice[p_idNFT] / 100) * 2;
            uint256 amountSeller = (s_salesPrice[p_idNFT] - amountFee);
            address payable a = payable(REWARDS_ADDRESS);
            a.transfer(amountFee);

            address payable b = payable(s_oldOwner[p_idNFT]);
            b.transfer(amountSeller);
       }else{
            require(IERC20(ERC20_ADDRESS).balanceOf(msg.sender) >= s_salesPrice[p_idNFT], "no enough balance");
            // 6% leaf fee to rewards contract
            uint256 amountFee = (s_salesPrice[p_idNFT] / 100) * 6;
            uint256 amountSeller = (s_salesPrice[p_idNFT] - amountFee);
